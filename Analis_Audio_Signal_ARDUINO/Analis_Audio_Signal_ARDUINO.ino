...
// сигнал
#define INPUT_GAIN 10    // коэффициент усиления входного сигнала
...
int sample = analogRead(A0); // або AUDIO_IN
  ...
  if (millis() - mainDelay > DELAY - 2) {     // итерация главного цикла
    ...
    // Розрахунок індекса з інверсією в межах кожної матриці
      byte matrixIndex = pos / 8;          // номер матриці
      byte columnInMatrix = pos % 8;       // позиція в матриці
      byte drawPos = matrixIndex * 8 + (7 - columnInMatrix);  // інверсія всередині матриці
      ...
      if (posLevel > 0) matrix.drawLine(drawPos, 7, drawPos, 7 - posLevel, HIGH);

      if (posLevel > 0 && posLevel > maxLevel[drawPos]) {    // если для этой полосы есть максимум, который больше предыдущего
        maxLevel[drawPos] = posLevel;                        // запомнить его
        timeLevel[drawPos] = millis();                       // запомнить время
      }

      // если точка максимума выше нуля (или равна ему) - включить пиксель
      if (MAX_DOTS && maxLevel[pos] >= 0) matrix.drawPixel(drawPos, 7 - maxLevel[drawPos], HIGH);
